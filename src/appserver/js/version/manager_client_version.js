...
function internal_DownloadShowUrl(value) {
    ...
    return String.format(
        "<a href='javascript:;'style='color: green;' onclick='download_log();'>" + value + "</a>&nbsp;&nbsp;&nbsp;" +
        '<a id="create_qrcode.info" href="javascript:void(0);" onclick="create_qrcode();return false;" style="color: green;">生成二维码</a>&nbsp;&nbsp;&nbsp;'+
        '<a id="qrcode.info" href="javascript:void(0);" onclick="qrcode();return false;" style="color: green;">显示二维码</a>'
    );
}
...
function create_qrcode() {
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            xtype: 'textfield',
            name:"url",
            //value: 'http://222.46.20.174:8081/DownLoadAndroid',
            value: 'http://172.16.2.9/DownLoadAndroid'
        }]
    });
    var win = new Ext.Window({
        title: '客户端二维码',
            ...
            handler: function () {
                if (formPanel.form.isValid()) {
                    formPanel.getForm().submit({
                        url :'../../ClientVersionAction_createQrcode.action',
                        method :'POST',
                        waitTitle :'系统提示',
                        waitMsg :'正在生成,请稍后...',
                        success : function(form,action) {
...
function qrcode() {
    new Ext.Window({
        ...
        items: [{
            ...
            autoEl: {
                tag: 'img',    //指定为img标签
                src: "../../ClientVersionAction_qrcode.action"       //指定url路径
            }
        }]
    }).show();
}
