Ext.onReady(function () {
    ...
    var record = new Ext.data.Record.create([
        ...
        {name: 'lardLord_ids', mapping: 'lardLord_ids'},
    ...
    var proxy = new Ext.data.HttpProxy({
        url: "../../CommunityDoorplateAction_findByAccount.action"
    });
    ...
    var colM = new Ext.grid.ColumnModel([
        ...
        //{header: "户主", dataIndex: "lardlord", align: 'center', sortable: true, menuDisabled: true},
        {header: "类型", dataIndex: "status", align: 'center', sortable: true, menuDisabled: true,renderer:show_status},
        //{header: "关注度", dataIndex: "lardlord_attention", align: 'center', sortable: true, menuDisabled: true,renderer:show_attention},
        //{header: "户主身份证", dataIndex: "lardlord_idcard", align: 'center', sortable: true, menuDisabled: true},
        {
            header: '操作标记',
            dataIndex: "personIds",
            align: 'center',
...
function show_room_status(value,o,r){
    if (value == '0') {
        return '<span style="color:green;">自住房</span>';
    } else if(value == '1') {
        return '<span style="color:red;">出租房</span>';
    }
}
...
function show_flag(value,o,r) {
    var lardLord_ids = r.get("lardLord_ids");
    ...
    return String.format(
        ...
        '<a id="lardlord.info" href="javascript:void(0);" onclick="lardlord(\''+lardLord_ids+'\');return false;" style="color: green;">户主</a>&nbsp;&nbsp;&nbsp;' +
        ...
        items: [
            {
                ...
                fieldLabel: "门牌",
                id:'doorplate.insert.info',
                name: 'communityDoorplate.doorplate',
                regex:/^[0-9]*$/,
                regexText:'请输入数字',
                emptyText: '请输入门牌号(数字)'/*,
                listeners:{
                    blur:function(){
                        ...
                        var unit = Ext.getCmp('unit.info').getValue();
                        var doorplate = this.getValue();
                        if(doorplate.length>0){
                            ...
                            myMask.show();
                            Ext.Ajax.request({
                                url: '../../CommunityDoorplateAction_checkDoorplate.action',
                                params :{build:build,unit:unit,doorplate:doorplate},
                                method:'POST',
                                success : function(r,o) {
                                    ...
                                    if(msg != 'true'){
                                        Ext.MessageBox.show({
                                            ...
                                            fn:function(e){
                                                if(e=='ok'){
                                                    Ext.getCmp('doorplate.insert.info').setValue('');
                                                }
                                            }
                                        });
                                    }
                ...
                fieldLabel:'出租/自住',hiddenName:'communityDoorplate.status',
                xtype:'combo',
                mode:'local',
                emptyText :'--请选择--',
                editable : false,
                ...
                handler: function () {
                    if (formPanel.form.isValid()) {
                        formPanel.getForm().submit({
                            url: '../../CommunityDoorplateAction_insert.action',
                            method: 'POST',
                            waitTitle: '系统提示',
                            waitMsg: '正在保存,请稍后...',
        ...
        bbar: [
            '->',
            {
                ...
                handler: function () {
                    if (formPanel.form.isValid()) {
                        formPanel.getForm().submit({
                            url: '../../CommunityDoorplateAction_update.action',
                            method: 'POST',
                            waitTitle: '系统提示',
                            waitMsg: '正在保存,请稍后...',
                            success: function (form, action) {
...
function delete_rule() {
    ...
    if (!recode) {
        ...
        Ext.Msg.confirm("提示", "确定删除这条记录？", function (sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url: "../../CommunityDoorplateAction_remove.action",
                    timeout: 20 * 60 * 1000,
                    method: "POST",
                    params: {
                        id: recode.get("id")
...
function show_status(value){
    if (value == "1") {
        return String .format('<a href="javascript:void(0);" onclick="zz_doorplate();return false;" style="color: red;">出租房</a>');
    } else {
        return String .format('<a href="javascript:void(0);" onclick="cz_doorplate();return false;" style="color: green;">自住房</a>');
    }
}
...
function qrcode() {
    ...
    if (!recode) {
        ...
        var url = "../../CommunityDoorplateAction_qrcode.action?id="+recode.get("id");
        new Ext.Window({
            title: "二维码",
            width: 300,
            layout: 'fit',
...
function lardlord(ids) {
    ...
    if(ids==undefined||ids.length==0){
        ...
        Ext.Ajax.request({
            url: '../../LardLordAction_findByIds.action?ids=' + ids,
                ...
                for(var i = 0; i < total; i ++) {
                    ...
                    var userPanel = new Ext.Panel({
                        ...
                        items:[{
                            ...
                            autoEl: {
                                tag: 'img',    //指定为img标签
                                src: "../../LardLordAction_loadLardHead.action?id=" + row.id       //指定url路径
                            }
                        },{
                            columnWidth:.1,
                    ...
                    userPanel2.add(new Ext.form.DisplayField({ id:'xzz.info' + row.id , fieldLabel:'现住地址',value: row.xzz }));
                    userPanelsun.add(new Ext.form.DisplayField({ id:'status.info' + row.id , fieldLabel:'类型',value:show_status_lardlord(row.status) }));
...
function show_status_lardlord(status){
    if (status == "1") {
        return '<span style="color:red;">房东</span>';
    } else {
...
function zz_doorplate(){
    var grid_panel = Ext.getCmp("grid.info");
    var recode = grid_panel.getSelectionModel().getSelected();
    if (!recode) {
        Ext.Msg.alert("提示", "请选择一条记录!");
    } else {
        Ext.Msg.confirm("提示", "确定更新为自住房？", function (sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url: "../../CommunityDoorplateAction_zzDoorplate.action",
                    timeout: 20 * 60 * 1000,
                    method: "POST",
                    params: {
                        id: recode.get("id")
...
function cz_doorplate(){
    var grid_panel = Ext.getCmp("grid.info");
    var recode = grid_panel.getSelectionModel().getSelected();
    if (!recode) {
        Ext.Msg.alert("提示", "请选择一条记录!");
    } else {
        Ext.Msg.confirm("提示", "确定更新为出租房？", function (sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url: "../../CommunityDoorplateAction_czDoorplate.action",
                    timeout: 20 * 60 * 1000,
                    method: "POST",
                    params: {
                        id: recode.get("id")
