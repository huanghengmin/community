Ext.onReady(function () {
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        buttons: [new Ext.Toolbar.Spacer({width: 10}), {
            ...
            handler: function () {
                if (formPanel.form.isValid()) {
                    formPanel.getForm().submit({
                        url: '../../LardLordOCRAction_saveOcr.action',
                        method: 'POST',
                        waitTitle: '系统提示',
                        waitMsg: '正在保存,请稍后...',
            ...
            handler: function () {
                Ext.Ajax.request({
                    url: '../../LardLordOCRAction_nextOcr.action',
                    method: 'POST',
...
function nextPic() {
    Ext.Ajax.request({
        url: "../../LardLordOCRAction_getPic.action",
        success: function (response) {
            var result = Ext.util.JSON.decode(response.responseText);
            var success = result.success;
            var msg = result.msg;
                    ...
                    items: [
                        {
                            ...
                            autoEl: {
                                tag: 'img',    //指定为img标签
                                src: '../LardLordOCRAction_loadPic.action?id=' + id   //指定url路径
                            }
                        }
                    ]
                });
